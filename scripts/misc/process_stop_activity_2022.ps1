$STOP_ACTIVITY = 'scripts/misc/SFMTA_Bus_Stop_Activity_Fall_2022.csv'
...
$MAX_ONS_PER_BSID = @"
    SELECT
        BS_ID,
        STOP_NAME,
        SUM([Sum of AVG_VEH_ONS]) AS max_ons
    FROM
        "$STOP_ACTIVITY"
    GROUP BY BS_ID
    ORDER BY BS_ID
...
$FULL_QUERY = @"
    SELECT
        ...
        max_ons,
        log_max_ons
    FROM ($LOG_ONS_PER_BSID) AS ons
    JOIN "sfmta_gtfs/stops.txt" ON (stop_id = BS_ID)
    ORDER BY BS_ID
"@
